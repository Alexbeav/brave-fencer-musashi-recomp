...
#
# Writes: dist/brave-fencer-musashi-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix brave-fencer-musashi-recomp \
  --exe-name Brave_Fencer_Musashi_Recompiled \
  --display-name "Brave Fencer Musashi Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Brave Fencer Musashi disc" \
